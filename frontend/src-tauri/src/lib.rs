...
async fn stop_voice_loop_if_running() {
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(5))
        .build();
    if let Ok(client) = client {
        let _ = client
            .post(format!("{}/v1/voice-loop/stop", api_base()))
            .send()
            .await;
    }
}

async fn shutdown_runtime(backend: SharedBackend, status: SharedStatus) {
    stop_voice_loop_if_running().await;
    backend.lock().await.stop_all().await;
    let mut current = status.lock().await;
    *current = SetupStatus {
        phase: "stopped".into(),
        detail: "Runtime stopped. Start it again when you want to use JARVIS.".into(),
        ollama_ready: false,
        server_ready: false,
        model_ready: false,
        error: None,
...
#[tauri::command]
async fn get_desktop_runtime_status(
    app: AppHandle,
    ...
    Ok(DesktopRuntimeStatus {
        ...
        project_root: find_project_root().map(|path| path.display().to_string()),
        window_visible,
        hide_to_tray: true,
        setup: status.lock().await.clone(),
    })
}
...
async fn start_backend(
...
) -> Result<(), String> {
    {
        ...
        *current = SetupStatus {
            phase: "starting".into(),
            detail: "Starting JARVIS runtime...".into(),
            ollama_ready: false,
            server_ready: false,
            model_ready: false,
            error: None,
...
async fn stop_backend(
    ...
    shutdown_runtime(backend.inner().clone(), status.inner().clone()).await;
    Ok(())
}
...
async fn restart_backend(
    ...
    shutdown_runtime(backend.inner().clone(), status.inner().clone()).await;
    {
        ...
        current.detail = "Restarting JARVIS runtime...".into();
    }
    let b = backend.inner().clone();
    let s = status.inner().clone();
    tauri::async_runtime::spawn(boot_backend(b, s));
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let start_runtime =
                MenuItemBuilder::with_id("start_runtime", "Start Runtime").build(app)?;
            let restart_runtime =
                MenuItemBuilder::with_id("restart_runtime", "Restart Runtime").build(app)?;
            let stop_runtime =
                MenuItemBuilder::with_id("stop_runtime", "Stop Runtime").build(app)?;
                ...
                .on_menu_event(move |app, event| match event.id().as_ref() {
                    ...
                    "start_runtime" => {
                        ...
                        tauri::async_runtime::spawn(async move {
                            ...
                            *current = SetupStatus {
                                phase: "starting".into(),
                                detail: "Starting JARVIS runtime from tray...".into(),
                                ollama_ready: false,
                                server_ready: false,
                                model_ready: false,
                                error: None,
                    ...
                    "restart_runtime" => {
                        ...
                        let status = app.state::<SharedStatus>().inner().clone();
                        tauri::async_runtime::spawn(async move {
                            shutdown_runtime(backend.clone(), status.clone()).await;
                            {
                                let mut current = status.lock().await;
                                current.phase = "restarting".into();
                                current.detail = "Restarting JARVIS runtime from tray...".into();
                            }
                            boot_backend(backend, status).await;
                        });
                    }
                    "stop_runtime" => {
                        ...
                        let status = app.state::<SharedStatus>().inner().clone();
                        tauri::async_runtime::spawn(async move {
                            shutdown_runtime(backend, status).await;
                        });
                    }
                    "quit" => {
                        let backend = app.state::<SharedBackend>().inner().clone();
