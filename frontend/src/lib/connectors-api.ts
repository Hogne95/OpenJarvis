...
export async function listConnectors(): Promise<ConnectorInfo[]> {
  const res = await fetchWithTimeout(`${getBase()}/v1/connectors`, {}, 7000);
...
export async function getConnector(id: string): Promise<ConnectorInfo> {
  const res = await fetchWithTimeout(`${getBase()}/v1/connectors/${encodeURIComponent(id)}`, {}, 7000);
...
export async function connectSource(id: string, req: ConnectRequest): Promise<ConnectorInfo> {
  const res = await fetchWithTimeout(`${getBase()}/v1/connectors/${encodeURIComponent(id)}/connect`, {
    method: 'POST',
...
export async function disconnectSource(id: string): Promise<void> {
  const res = await fetchWithTimeout(`${getBase()}/v1/connectors/${encodeURIComponent(id)}/disconnect`, {
    method: 'POST',
  }, 7000);
...
export async function getSyncStatus(id: string): Promise<SyncStatus> {
  const res = await fetchWithTimeout(`${getBase()}/v1/connectors/${encodeURIComponent(id)}/sync`, {}, 7000);
...
export async function triggerSync(id: string): Promise<{ connector_id: string; chunks_indexed: number; status: string }> {
  const res = await fetchWithTimeout(`${getBase()}/v1/connectors/${encodeURIComponent(id)}/sync`, {
    method: 'POST',
  }, 10000);
  if (!res.ok) {
