...
echo.
echo WSL command 1: run shared backend startup with Python-aware voice extras
echo WSL command 2: uv run jarvis serve --port 8000
echo WSL command 3: npm run dev
echo.

start "OpenJarvis Backend" cmd /k %UBUNTU_CMD% "cd \"%ROOT_WSL%\" && bash scripts/start_openjarvis_backend.sh \"%ROOT_WSL%\"; exec bash"
