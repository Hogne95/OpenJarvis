...
Write-Host "WSL command 1: uv sync --extra server --extra speech --extra speech-live --extra speech-wake --extra speech-tts-local"
Write-Host "WSL command 2: uv run jarvis serve --port 8000"
Write-Host "WSL command 3: npm install && npm run dev"
Write-Host ""

Start-Process cmd.exe -ArgumentList '/k', "wsl.exe bash -lc ""cd '$rootWsl' && uv sync --extra server --extra speech --extra speech-live --extra speech-wake --extra speech-tts-local && uv run jarvis serve --port 8000; exec bash"""
